...
#image = Image.open(r"D:\Downloads1\Airbnb.png")
image = Image.open(r"C:\Users\ADMIN\Desktop\New1folder\Airbnb.png")
...
if Menu == "Home":
    ...
    with col2:
        st.markdown("<h1 style='text-align: right; color: green;'>Airbnb Data Analysis and Exploration</h1>", unsafe_allow_html=True)

        #st.markdown("<h2 style='text-align: right; color: blue;'>A User-Friendly Tool Using Streamlit</h2>", unsafe_allow_html=True)


    st.image(r"C:\Users\ADMIN\OneDrive\Pictures\airbnb-swot-analysis-infographic-template.webp ")
    st.write(" ")
...
# OVERVIEW PAGE
if Menu == "Overview":
    tab1,tab2 = st.tabs([":green[DATA]", ":blue[TOP CHARTS]"])

    # DISPLAYING SAMPLE DATA OF MDB AND DATAFRAME
    with tab1:
        col1,col2 = st.columns([1,3])
        if col1.button("Sample data"):
            col1.write(col.find_one())
        if col2.button("Dataframe"):
            col1.write(col.find_one())
            col2.write(df)
